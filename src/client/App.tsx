...
import type { DraftPreview } from "./components/DraftReviewModal.js";
...
const InterviewWizard = lazy(async () => {
  const module = await import("./components/InterviewWizard.js");
  return { default: module.InterviewWizard };
});
const DraftReviewModal = lazy(async () => {
  const module = await import("./components/DraftReviewModal.js");
  return { default: module.DraftReviewModal };
});
const ResultsPanel = lazy(async () => {
  const module = await import("./components/ResultsPanel.js");
    ...
    style: {
      stroke:
        flow.id === selectedFlowId
          ? "#53d7ff"
          : flow.reviewStatus === "needs-review"
            ? "#ffbf69"
...
function AppShell(): React.ReactNode {
  ...
  const runAnalysis = async (): Promise<void> => {
    ...
    if (unreviewedCount > 0) {
      setRightMode("design");
      setMessage(
        `${unreviewedCount} generated components or flows require confirmation before analysis.`,
      );
      return;
    }
    setRunning(true);
  ...
  const importModel = async (file: File): Promise<void> => {
    try {
      const { importArchitecture, MAX_IMPORT_BYTES } = await import("../importers/index.js");
      if (file.size > MAX_IMPORT_BYTES)
        throw new Error("Architecture imports are limited to 2 MB.");
      const imported = importArchitecture(file.name, await file.text());
      setDraftPreview({
        sourceLabel: imported.formatLabel,
        model: imported.model,
        warnings: imported.warnings,
        sourceCount: imported.sourceCount,
      });
      setMessage(`${imported.formatLabel} parsed into a review draft.`);
    } catch (error: unknown) {
        ...
        <header className="app-header">
          ...
          <div className="header-context">
            <span className="mode-pill">
              <Sparkles size={13} />{" "}
              {model.systemKind === "auto" ? "Auto-detect" : model.systemKind}
            </span>
            <span className="header-system-name">{model.name}</span>
          </div>
          <nav className="header-actions">
            <button
              className="button button--ghost button--interview"
              type="button"
              onClick={() => setInterviewOpen(true)}
            >
              <MessageSquareText size={15} /> Interview
            </button>
            ...
            <button
              ...
              type="button"
              title="Import Argus, OpenAPI, Compose, Kubernetes, Terraform or MCP"
        ...
        <main className="workspace">
          ...
          <aside className="right-panel">
            ...
            {rightMode === "results" && analysis ? (
              <Suspense fallback={<div className="panel-empty">Loading findings…</div>}>
      ...
      <Suspense fallback={null}>
        ...
        {draftPreview ? (
          <DraftReviewModal
            ...
            onApply={() => {
              loadSample(draftPreview.model);
              setMessage(
                `${draftPreview.sourceLabel} applied. Confirm generated evidence before accepting findings.`,
              );
